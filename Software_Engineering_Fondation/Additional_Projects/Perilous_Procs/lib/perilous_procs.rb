...
# If an original word returns true for multiple key procs, then only the value proc 
# that appears earliest in the hash should be applied.
...
def picky_procipher(sentence, hash)
	...
	words.each.with_index do |word, index|
		hash.each do |prc_keys, prc_vals|
			 if prc_keys.call(words[index])
				 words[index] = prc_vals.call(words[index])
				 break
			 end
		end
	end
